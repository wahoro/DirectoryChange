...
#ifdef _DEBUG
				//if (pNotification->m_szFileName1)
				//{
				//	OutputDebugString(pNotification->m_szFileName1);
				//	OutputDebugString(L"\n");
				//}
				//if(pNotification->m_szFileName2)
				//{
				//	OutputDebugString(pNotification->m_szFileName2);
				//	OutputDebugString(L"\n");
				//}
...
//
{
	...
	if( m_nNumIncludeFilterSpecs == 1 )
	{
		return _PathMatchSpec(CString(szFileName).MakeUpper(), m_szIncludeFilter)? true : false;
	}
	else
	{
		TCHAR ** ppTmp = (TCHAR**)m_szIncludeFilter;
		for(int i(0); i < m_nNumIncludeFilterSpecs; ++i )
		{
			if( _PathMatchSpec(CString(szFileName).MakeUpper(), *ppTmp++) )
				return true;
		}
		return false;
	}
...
//
{
	...
	if( m_nNumExcludeFilterSpecs == 1 )
	{
		if( _PathMatchSpec(CString(szFileName).MakeUpper(), m_szExcludeFilter) )
