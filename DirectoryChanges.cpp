...
{ 
	return InterlockedCompareExchange((volatile LONG *)&m_nRefCnt,0,0);
}
...
//
//
void CDirectoryChangeHandler::On_FileAdded(const CString & strFileName,TARGET_TYPE eTargetType)
{ 
	return;
}

void CDirectoryChangeHandler::On_FileRemoved(const CString & strFileName,TARGET_TYPE eTargetType)
{
	return;
}

void CDirectoryChangeHandler::On_FileModified(const CString & strFileName,TARGET_TYPE eTargetType)
{
	return;
}

void CDirectoryChangeHandler::On_FileNameChanged(const CString & strOldFileName, const CString & strNewFileName,TARGET_TYPE eTargetType)
{
	return;
}

bool CDirectoryChangeHandler::On_FilterNotification(DWORD dwNotifyAction, LPCTSTR szFileName, LPCTSTR szNewFileName,TARGET_TYPE eTargetType)
{
	return true;
}

//因为这个函数被调用时，意味着strDirectoryName不会再被监视，比较重要，所以这个函数必须在子类被重载，否则默认的处理函数会直接让程序崩溃掉
...
**************************************************************/
{
	...
	CDirWatchInfo * pDirInfo = new CDirWatchInfo( hDir,
												strDirToWatch,
												pChangeHandler, 
												dwChangesToWatchFor, 
												bWatchSubDirs, 
												m_bAppHasGUI, 
												CString(szIncludeFilter).MakeUpper(), 
												CString(szExcludeFilter).MakeUpper(), 
												m_dwFilterFlags);
	if( !pDirInfo )
	{
	...
	m_hCompPort = CreateIoCompletionPort(hDir, 
										...
										(ULONG_PTR)pDirInfo, //the completion 'key'... this ptr is returned from GetQueuedCompletionStatus() when one of the events in the dwChangesToWatchFor filter takes place
										0);
	if( m_hCompPort == NULL )
	{
