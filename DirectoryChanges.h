...
#define READ_DIR_CHANGE_BUFFER_SIZE (1024*1024) //1MB
...
{
public:
	DWORD m_dwMonitorType;

	typedef enum{eIsDir=0,eIsFile=1,eUnknown=2}TARGET_TYPE;
...
public:
	...
	virtual bool On_FilterNotification(DWORD dwNotifyAction, LPCTSTR szFileName, LPCTSTR szNewFileName,TARGET_TYPE eTargetType);




	//鉴于这个函数的重要性，所以这个函数必须被重载，否则进程会崩溃掉
	...
	virtual void On_WatchStarted(DWORD dwError, const CString & strDirectoryName){return;};
	//
	...
	virtual void On_WatchStopped(const CString & strDirectoryName);//这个函数必须被重载，否则让整个程序崩溃
	//
...
private:
	...
	virtual void On_FileAdded(const CString & strFileName)
	{
		...
		TARGET_TYPE eType=eUnknown;

		if (dwAttr!=0xFFFFFFFF)
		{
			if (eType & FILE_ATTRIBUTE_DIRECTORY)
			{
				eType=eIsDir;
			}else
			{
		...
		this->On_FileAdded(strFileName,eType);
	} //=0;
			//
			//	On_FileAdded()
			//
	...
	virtual void On_FileRemoved(const CString & strFileName)
	{
		...
		this->On_FileRemoved(strFileName,eType);
	}
			//
			//	On_FileRemoved()
			//
	...
	{
		DWORD dwAttr=::GetFileAttributes( strOldFileName);

		TARGET_TYPE eType=eUnknown;

		if (dwAttr==0xFFFFFFFF)
		{
			dwAttr=::GetFileAttributes( strNewFileName);
		}
		...
		this->On_FileNameChanged(strOldFileName,strNewFileName,eType);
	}
			//
			//	On_FileNameChanged()
			//
	...
	virtual void On_FileModified(const CString & strFileName)
	{
		...
		this->On_FileModified(strFileName,eType);
	}
			//
			//	On_FileModified()
			//
	...
	virtual bool On_FilterNotification(DWORD dwNotifyAction, LPCTSTR szFileName, LPCTSTR szNewFileName)
	{
		...
		if (dwAttr==0xFFFFFFFF)
		{
			dwAttr=::GetFileAttributes( szNewFileName);
		}
		...
		return this->On_FilterNotification(dwNotifyAction,szFileName,szNewFileName,eType);
	}
			//
